            ...
            fluidRow(
                column(
                    ...
                    tags$small(
                        ...
                        tags$a("Air Quality Index Hub",
                            href = "https://aqihub.info/indices/hong-kong",
                            target = "_blank"
                        )
                    ),
                    hr()
                ...
                column(
                    ...
                    tags$ul(
                        ...
                        tags$li(
                            ...
                            paste0(
                                month.name[lubridate::month(min(aq_all$date, na.rm = TRUE))], " ",
                                lubridate::year(min(aq_all$date, na.rm = TRUE))
                            ), " to ",
                            paste0(
                                month.name[lubridate::month(max(aq_all$date, na.rm = TRUE))], " ",
            ...
            fluidRow(
                    ...
                    tags$ul(
                        tags$li(
                            ...
                            tags$code("shiny, dplyr, lubridate, plotly, sf, leaflet, DT, shinyjs, stars, raster, rstudioapi")
                        ),
                        br(),
                        tags$li(
                            tags$strong("Data Processing"), br(),
                            tags$small(
                                "• Import and merge 60 monthly CSV files (2021-2025)", br(),
                        ...
                        tags$li(
                            ...
                            tags$small(
                                ...
                                br(),
                                tags$strong("Kriging (Manual):"), br(),
                                "  • Variogram parameters set manually", br(),
                                "  • Sill = 70% of AQHI variance (captures most spatially structured variation)", br(),
                                "  • Nugget = 30% of AQHI variance (accounts for measurement error and small-scale variability)", br(),
                                "  • Range = median of inter-station distances", br(),
                                br(),
                                ...
                                "  • Monthly: rasters for each month (2021-2025)", br(),
                                "  • Annual: yearly averages", br(),
                                "  • Hourly: average for each hour across all years", br(),
                                "  • Resolution: 500m grid", br(),
                                "  • Total: includes Kriging, Kriging (manual), and IDW outputs for all levels"
                            )
                        )
                    )
                ),
                column(
                    ...
                    tags$ul(
                        tags$li(
                            ...
                            tags$small(
                                "Source: ", tags$a("Open Data Portal of the Government of Hong Kong",
                        ...
                        tags$li(
                            ...
                            tags$small(
                                "Source: ", tags$a("Environmental Protection Department (EPD) Hong Kong",
            ...
            sidebarLayout(
                sidebarPanel(
                    ...
                    radioButtons("spatial_method",
                        ...
                        choices = c("Kriging (fitted variogram)" = "kriging_fit", "Kriging (manual variogram)" = "kriging_manual", "IDW" = "idw"),
                        selected = "kriging_fit"
                    ),
                    hr(),
...
server <- function(input, output, session) {
    ...
    spatial_raster <- reactive({
        ...
        if (!file.exists(filepath)) {
            print(paste("Raster file not found:", filepath))
            return(NULL)
        }
    ...
    output$spatial_title <- renderText({
        method_name <- switch(input$spatial_method,
            "kriging_fit" = "Kriging (fitted variogram)",
            "kriging_manual" = "Kriging (manual variogram)",
            "idw" = "IDW",
            "Unknown Method"
        )
        ...
        if (input$spatial_type == "month") {
            paste0(
                "Spatial Interpolation (", method_name, ") - ",
                input$spatial_month, " ", input$spatial_year
            )
        } else if (input$spatial_type == "annual") {
            paste0(
                "Spatial Interpolation (", method_name, ") - Annual Average ",
                input$spatial_year_annual
            )
        } else {
            paste0(
                "Spatial Interpolation (", method_name, ") - Hour ",
    ...
    output$spatial_map <- renderLeaflet({
        ...
        # Add raster layer
        m <- m %>%
            addRasterImage(r_raster, colors = pal, opacity = 0.4) %>%
            addLegend(
